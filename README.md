# International Petition – Version 1.0

This repository contains **Version 1.0** of an international petition addressing the  
**effects of illegitimate public debts on fundamental human rights**.

This document is published as a **stable reference text**, intended to support
analysis, public discussion, institutional review, and lawful civic initiatives.

---

## What this petition is

- A formal international petition
- A stable, versioned reference document (v1.0)
- A documentary basis for research, advocacy, and public debate
- A multilingual resource accessible to citizens, researchers, journalists, NGOs, and institutions

---

## Repository structure

### Petition texts (full versions)
The complete petition text is available in multiple languages:

📂 **petition-texts/**  
→ one `.txt` file per language, containing the full petition text.

---

## Available languages
(Language order does not imply legal precedence.)

## Available languages

- [Français](readme/README_fr.md)
- [English](readme/README_en.md)
- [Español](readme/README_es.md)
- [Deutsch](readme/README_de.md)
- [Italiano](readme/README_it.md)
- [Português](readme/README_pt.md)
- [Русский](readme/README_ru.md)
- [中文（简体）](readme/README_zh.md)
- ‎[العربية](readme/README_ar.md)


This petition is grounded in international law, United Nations treaties, and recognized legal doctrine.

- [English – full text](petition-texts/en.txt)
- [Français – texte intégral](petition-texts/fr.txt)
- [Español – texto completo](petition-texts/es.txt)
- [Deutsch – vollständiger Text](petition-texts/de.txt)
- [Italiano – testo completo](petition-texts/it.txt)
- [Português – texto completo](petition-texts/pt.txt)
- [Русский – полный текст](petition-texts/ru.txt)
- [中文（简体）– 完整文本](petition-texts/zh.txt)
‎[العربية – النص الكامل](petition-texts/ar.txt)


The following file paths provide direct access to the complete petition texts.
They are provided in a copy-friendly format for citation, sharing, and reuse.
📚 **Sources and legal foundations**  
👉 [Read the sources](sources.md)


## What this repository is NOT

- Not a legal judgment  
- Not a court filing  
- Not a call to illegal action  

This repository is a **documentary and preparatory resource**, published in accordance with international norms.

---

## Versioning

- Current version: **1.0**
- This version is considered stable and reference-ready.
- ## Additional documentation

- [Sources and references](sources.md)
- [About this project](about.md)
